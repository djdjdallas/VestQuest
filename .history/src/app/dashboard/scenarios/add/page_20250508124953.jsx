...
export default function AddScenarioPage() {
  ...
  const { grants: hookGrants, loading: hooksGrantLoading } = useGrants();

  // Fetch grants to use for default values and calculations
  useEffect(() => {
    const fetchGrants = async () => {
      try {
        if (hookGrants && hookGrants.length > 0) {
          setGrants(hookGrants);
        ...
        toast.error(
          "Failed to fetch grants. Default values may not be optimal."
        );
      } finally {
        setGrantsLoading(false);
      }
    ...
    fetchGrants();
  }, [supabase, hookGrants, hooksGrantLoading]);

  // Get default values for the form based on available grants
  ...
  const handleCreateScenario = async (scenarioData) => {
    ...
    try {
      ...
      if (previewData) {
        dataWithUserId.gross_proceeds = previewData.gross_proceeds;
        dataWithUserId.exercise_cost = previewData.exercise_cost;
        dataWithUserId.tax_liability = previewData.tax_liability;
        dataWithUserId.net_proceeds = previewData.net_proceeds;
        dataWithUserId.roi_percentage = previewData.roi_percentage;
        dataWithUserId.effective_tax_rate = previewData.effective_tax_rate || 0;
      }
          ...
          {previewData ? (
            <Card>
              ...
              <CardFooter className="border-t pt-4 text-xs text-muted-foreground">
                This is an estimate. Actual results may vary based on tax laws
                and other factors.
              </CardFooter>
            </Card>
          ) : (
