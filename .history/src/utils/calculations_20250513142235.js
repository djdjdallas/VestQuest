...
/**
 * Enhanced calculation functions for equity scenarios
 */
...
export function calculateVestedShares(grant, asOfDate = new Date()) {
  ...
  const vestingStart = grant.vesting_start_date
    ...
    : null;
  const vestingEnd = grant.vesting_end_date
  ...
  // Safety check - if missing critical data, return 0
  if (!vestingStart || !vestingEnd || !grant.shares) {
    return 0;
  }
  ...
  // If cliff date is specified and we're before cliff date
  ...
  const elapsedDays = (now - vestingStart) / (1000 * 60 * 60 * 24);
  const vestingRatio = Math.min(1, elapsedDays / totalVestingDays);
  ...
  // Simple linear vesting without cliff
  return Math.floor(vestingRatio * totalShares);
}
...
export function calculateScenarioResult(
  ...
  const shares =
    typeof sharesToExercise === "number"
      ? sharesToExercise
      : parseInt(sharesToExercise) || 0;
  const exitPrice =
    typeof exitValue === "number" ? exitValue : parseFloat(exitValue) || 0;
  const strikePrice =
    typeof grant.strike_price === "number"
      ? grant.strike_price
      : parseFloat(grant.strike_price) || 0;

  // Calculate exercise cost - what you pay to exercise the options
  const exerciseCost = calculateExerciseCost(shares, strikePrice);

  // Calculate gross proceeds - what you get from selling all shares
  const grossProceeds = shares * exitPrice;

  // Simplified tax calculation
  // In a real implementation this would be more complex based on:
  // - Grant type (ISO vs NSO vs RSU)
  // - Holding period (long-term vs short-term)
  // - Personal tax situation
  let taxLiability = 0;

  if (grant.grant_type === "ISO") {
    // Simplified ISO taxation - assume 25% tax on spread at exit
    const spreadAtExit = (exitPrice - strikePrice) * shares;
    taxLiability = spreadAtExit * 0.25;
  } else if (grant.grant_type === "NSO") {
    // Simplified NSO taxation - assume 35% on spread
    const spreadAtExercise = (grant.current_fmv - strikePrice) * shares;
    const capitalGains = (exitPrice - grant.current_fmv) * shares;
    taxLiability = spreadAtExercise * 0.35 + capitalGains * 0.2;
  } else if (grant.grant_type === "RSU") {
    // Simplified RSU taxation - assume 35% on full value
    taxLiability = grossProceeds * 0.35;
  } else {
    // Default case - apply 30% tax on gains
    const gain = grossProceeds - exerciseCost;
    taxLiability = gain > 0 ? gain * 0.3 : 0;
  }

  // Calculate net proceeds (what you get after costs and taxes)
  const netProceeds = grossProceeds - exerciseCost - taxLiability;

  // Calculate ROI percentage
  const roiPercentage =
    exerciseCost > 0 ? (netProceeds / exerciseCost) * 100 : 0;

  // Calculate effective tax rate
  const effectiveTaxRate = grossProceeds > 0 ? taxLiability / grossProceeds : 0;
