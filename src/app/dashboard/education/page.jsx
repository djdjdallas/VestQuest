...
export default function Education() {
  ...
  useEffect(() => {
    ...
    if (glossaryContent) glossaryContent.addEventListener("scroll", handleScroll);
      ...
      if (user) {
        try {
          ...
          if (progressData && progressData.length > 0) {
            ...
            localStorage.setItem('completedTopics', JSON.stringify(progressData[0].completed_topics || []));
            localStorage.setItem('bookmarkedTopics', JSON.stringify(progressData[0].bookmarked_topics || []));
            localStorage.setItem('learningProgress', JSON.stringify(progressData[0].learning_progress || {}));
            localStorage.setItem('quizResults', JSON.stringify(progressData[0].quiz_results || {}));
          }
        } catch (err) {
  ...
  }, [completedTopics, bookmarkedTopics, learningProgress, quizResults, supabase]);
    ...
    async function fetchEducationData() {
      ...
      try {
        ...
        const processedPaths = pathsData?.map(path => ({
          ...
          recommended_for: Array.isArray(path.recommended_for) ? path.recommended_for : [],
        ...
        if (processedPaths.length > 0) {
          const recommendedPath = processedPaths.find(path => 
  ...
  }, [completedTopics, bookmarkedTopics, learningProgress, quizResults, loading, saveUserProgress]);

  // Filter content based on search query and selected tags
  const filteredEducationContent = educationContent
    .filter(item => 
      // Search query filter
      (searchQuery 
  ...
  const filteredGlossaryTerms = glossaryTerms
    .filter(item =>
      searchQuery
  ...
  const getRecommendedContent = () => {
    ...
    const relatedTopics = incompleteTopics.filter(topic => 
      topic.related_terms && topic.related_terms.some(term => {
    ...
    if (recommended.length < 3) {
      const levelAppropriate = incompleteTopics.filter(topic => 
      ...
      recommended = [...recommended, ...levelAppropriate.slice(0, 3 - recommended.length)];
    }
  ...
  const getPersonalizedPath = () => {
    ...
    return learningPaths.find(path => path.level === educationLevel) || learningPaths[0];
  };
      ...
      >
        <div className="flex items-center space-x-2">
          <div className="flex items-center space-x-1">
            ...
            <Badge variant={educationLevel === 'beginner' ? 'default' : 'outline'} 
              className="cursor-pointer"
              onClick={() => setEducationLevel('beginner')}
            >
              Beginner
            </Badge>
            <Badge variant={educationLevel === 'intermediate' ? 'default' : 'outline'} 
              className="cursor-pointer"
              onClick={() => setEducationLevel('intermediate')}
            >
              Intermediate
            </Badge>
            <Badge variant={educationLevel === 'advanced' ? 'default' : 'outline'} 
              className="cursor-pointer"
            ...
            <div className="p-4 border rounded-md bg-muted/30">
              ...
              <div className="flex flex-wrap gap-2">
                {availableTopicTags.map(tag => (
                  <Badge 
                    key={tag}
                    variant={selectedTopicTags.includes(tag) ? 'default' : 'outline'}
                    className="cursor-pointer"
                    onClick={() => {
            ...
            <TabsContent value="paths" className="pt-4 relative">
              ...
              >
                  ...
                  {learningPaths.map(path => (
                    <div 
                      ...
                      className={`border rounded-lg p-4 cursor-pointer transition-all ${
                        currentLearningPath && currentLearningPath.id === path.id ? 
                    ...
                    >
                      <div className="flex justify-between">
                        ...
                        <Badge variant={path.level === educationLevel ? 'default' : 'outline'}>
                          {path.level}
                        </Badge>
                      </div>
                ...
                {currentLearningPath && (
                  <div className="space-y-4">
                    <div>
                      <h2 className="text-xl font-semibold">{currentLearningPath.title}</h2>
                      <p className="text-muted-foreground">{currentLearningPath.description}</p>
                      ...
                      {learningProgress[currentLearningPath.id] && (
                        <div className="mt-2">
                          <div className="flex justify-between text-sm mb-1">
                            ...
                            <span>
                              {Math.round(
                                (learningProgress[currentLearningPath.id].completed_modules?.length || 0) / 
                                (currentLearningPath.modules?.length || 1) * 100
                              )}%
                            </span>
                          </div>
                    ...
                    <div className="space-y-4">
                      {currentLearningPath.modules?.map((module, index) => {
                        const isCompleted = learningProgress[currentLearningPath.id]?.completed_modules?.includes(module.id);
                        const isActive = learningProgress[currentLearningPath.id]?.current_module === module.id ||
                          (!learningProgress[currentLearningPath.id]?.current_module && index === 0);
                            ...
                            <div className="flex justify-between items-start">
                              <div>
                                <h3 className="font-medium flex items-center">
                                  {isCompleted && <Award className="h-4 w-4 text-primary mr-1" />}
                              ...
                              <Button 
                                ...
                                onClick={() => {
                                  ...
                                  setLearningProgress(prev => {
                                    const pathProgress = prev[currentLearningPath.id] || {
                                      completed_modules: [],
                                      current_module: null
                                    };
                                  ...
                                  if (module.related_concepts) {
                                    module.related_concepts.forEach(conceptId => {
                                      markConceptViewed(conceptId);
                                    });
                                  }
                                }}
                              >
                                {isCompleted ? "Completed" : isActive ? "Continue" : "Start"}
                              </Button>
                            </div>
                            
                            {isActive && (
                              <div className="mt-4 space-y-3">
                                {module.content_sections?.map((section, sectionIndex) => (
                                    ...
                                    setLearningProgress(prev => {
                                      ...
                                      return {
                                        ...
                                        [currentLearningPath.id]: {
                                          ...
                                          current_module: 
                                            currentLearningPath.modules[index + 1]?.id || 
                                            null
                                        }
                                      };
                                    });
            ...
            <TabsContent value="decisionGuides" className="space-y-6 pt-4">
              <div className="grid gap-6 md:grid-cols-3">
                ...
                <div className="space-y-2 border rounded-lg p-4 hover:border-primary/50 transition-all">
                  ...
                  <p className="text-muted-foreground">
                    Understand the tax implications of your equity and develop strategies
                    to minimize your tax burden over time.
                  </p>
                  <Button asChild className="w-full mt-2">
              ...
              <div className="mt-8 border rounded-lg p-6">
                <h2 className="text-xl font-medium mb-4">Quick Decision Helper</h2>
                <p className="mb-4">
                  Answer a few questions to get a quick recommendation for your equity situation:
                </p>
                
                <div className="space-y-4">
                  {/* This would be a more interactive component in the full implementation */}
                  <div className="grid grid-cols-2 gap-4">
                    <Button variant="outline" size="lg" className="h-auto py-4 flex flex-col items-center justify-center">
                      <span className="font-medium">I need help deciding whether to exercise my options</span>
                      <span className="text-sm text-muted-foreground mt-1">Early exercise, post-vest exercise, or hold</span>
                    </Button>
                    
                    <Button variant="outline" size="lg" className="h-auto py-4 flex flex-col items-center justify-center">
                      <span className="font-medium">I need help understanding my equity grant</span>
                      <span className="text-sm text-muted-foreground mt-1">Vesting schedule, grant type, and value</span>
                    </Button>
                    
                    <Button variant="outline" size="lg" className="h-auto py-4 flex flex-col items-center justify-center">
                      <span className="font-medium">I need help with tax planning</span>
                      <span className="text-sm text-muted-foreground mt-1">AMT, QSBS, 83(b), and more</span>
                    </Button>
                    
                    <Button variant="outline" size="lg" className="h-auto py-4 flex flex-col items-center justify-center">
                      <span className="font-medium">I'm considering leaving my company</span>
                      <span className="text-sm text-muted-foreground mt-1">Post-termination exercise window and considerations</span>
                    </Button>
                  </div>
                </div>
              </div>
